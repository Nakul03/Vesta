...
const SolarSystem = () => {
  ...
  useEffect(() => {
    ...
    const outlinePass = new OutlinePass(new THREE.Vector2(currentMount.clientWidth, currentMount.clientHeight), scene, camera);
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(currentMount.clientWidth, currentMount.clientHeight), 1, 0.4, 0.85);
    ...
    currentMount.appendChild(guiContainer);
    ...
    function identifyPlanet(clickedObject) {
            if (clickedObject.material === mercury.planet.material) {
              ...
              offset = 10;
              return pluto;
            } else if (earthAsteroids.map(a => a.mesh).includes(clickedObject)) {
                offset = 10;
                return {planet: clickedObject, name: `Asteroid-${clickedObject.id}`};
    ...
    function showPlanetInfo(planet) {
      ...
      if (planet.startsWith('Asteroid-')) {
        name.innerText = 'Asteroid';
        details.innerText = 'This is a specific asteroid. More details can be added here.';
      } else {
    ...
    const earthAsteroid2 = {
      modelPath: './../asteroids/Asteroid_1e.glb',
      scale: 0.2,
      orbitRadius: 12,
      orbitSpeed: 0.0008,
      mesh: null
    ...
    marsMoons.forEach(moon => {
      loadObject(moon.modelPath, moon.position, moon.scale, function(gltf) {
    ...
    // Load Asteroids for Earth
    const earthAsteroid = {
      modelPath: './../asteroids/asteroidPack.glb',
      scale: 0.3,
      orbitRadius: 15,
      orbitSpeed: 0.001 * settings.accelerationOrbit,
      mesh: null
    ...
    loadObject(earthAsteroid.modelPath, 0, earthAsteroid.scale, function(gltf) {
      const asteroidModel = gltf.scene.children[0];
      const colors = [0x525252, 0xA8422D, 0xA8A9AD, 0x770F05, 0x808080, 0x525252];
      for (let i = 0; i < 5; i++) {
        const asteroidMesh = asteroidModel.clone();
        const angle = (i / 6) * Math.PI * 2;
        const x = 15 * Math.cos(angle);
        const z = 15 * Math.sin(angle);
        asteroidMesh.position.set(x, 0, z);
        earth.planetSystem.add(asteroidMesh);
        
        const newEarthAsteroid = {
          mesh: asteroidMesh,
          orbitSpeed: 0.001 * settings.accelerationOrbit
        };
        earthAsteroids.push(newEarthAsteroid);

        asteroidMesh.traverse(function (child) {
          if (child.isMesh) {
            const newMaterial = child.material.clone();
            newMaterial.color.set(colors[i]);
    ...
    loadObject(earthAsteroid2.modelPath, 0, earthAsteroid2.scale, function(gltf) {
      console.log("Loaded earthAsteroid2 model");
      earthAsteroid2.mesh = gltf.scene;
      earth.planetSystem.add(earthAsteroid2.mesh);
    });
      ...
      const planetData = {
        ...
        'Asteroid': {
            ...
            rotation: 'N/A',
            orbit: 'N/A',
            distance: '~10 km from Earth',
            moons: '0',
            info: 'A rocky object orbiting the Earth.'
        }
    };
    ...
    if (earthAsteroid2.mesh) {
      const time = performance.now();
      const angle = time * earthAsteroid2.orbitSpeed;
      const x = earth.planet.position.x + earthAsteroid2.orbitRadius * Math.cos(angle);
      const z = earth.planet.position.z + earthAsteroid2.orbitRadius * Math.sin(angle);
      earthAsteroid2.mesh.position.set(x, 0, z);
      earthAsteroid2.mesh.rotation.y += 0.01;
      console.log("Asteroid position:", earthAsteroid2.mesh.position);
    }
    ...
    if (earthAsteroids.length > 0) {
        earthAsteroids.forEach((asteroid, index) => {
            const time = performance.now() * asteroid.orbitSpeed;
            const initialAngle = (index / 5) * Math.PI * 2;
            const angle = initialAngle + time;
            asteroid.mesh.position.x = 15 * Math.cos(angle);
            asteroid.mesh.position.z = 15 * Math.sin(angle);
    ...
    return () => {
      ...
      currentMount.removeChild(guiContainer);
      gui.destroy();
    };
  }, []);
  ...
  return (
    <div ref={mountRef} style={{ width: '100%', height: '100vh' }}>
      <div id="planetInfo" style={{ display: 'none', position: 'absolute', top: '10px', left: '10px', color: 'white', backgroundColor: 'rgba(0,0,0,0.5)', padding: '10px' }}>
        <h2 id="planetName"></h2>
        <pre id="planetDetails"></pre>
        <button onClick={() => window.closeInfo()}>Close</button>
      </div>
    </div>
  );
};
